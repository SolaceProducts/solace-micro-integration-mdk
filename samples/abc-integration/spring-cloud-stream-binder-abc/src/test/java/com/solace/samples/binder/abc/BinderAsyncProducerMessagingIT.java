...
/**
 * Binder integration tests that use a TestApplication to test the async producer bindings. These
 * tests create an application context and an instance of the abc service running in a container.
 * <p>
 */
@ExtendWith(MockitoExtension.class)
@ExtendWith(OutputCaptureExtension.class)
@TestMethodOrder(MethodOrderer.OrderAnnotation.class)
class BinderAsyncProducerMessagingIT extends AbstractBaseWithOnTheFlyContainerIT {
  ...
  protected String[] getActiveProfiles() {
    return new String[]{"async-producer"};
  }

  /**
   * Test that the producer binding can successfully push messages  an asynchronous manner to the
   * Abc service.
   * <p>
  ...
  /**
   * Test that the producer binding can handle a failed async publish scenario.
   * <p>
  ...
  @Test
  @Order(3)
  void testProducerBindingFailedPublish(@Mock PublishAcknowledgmentCallback ackCallback)
  ...
  /**
   * Test that the producer binding can successfully push messages to a dynamic destination. The
   * presence of the scst_targetDestination header indicates that the message should be sent to a
   * dynamic destination.
   */
  @Test
  @Order(4)
