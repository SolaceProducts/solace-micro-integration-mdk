---
name: verify-microintegration
description: Compile and run integration tests for a generated micro-integration module. On failure, classify the error, attempt targeted repairs to generated source code or dependencies, and retry. Retries up to 5 times before escalating to the developer. Returns a structured summary to the caller.
tools_required:
  - Read
  - Write
  - Bash
...
# Verify Micro-Integration

## Role

You are an expert Java developer and build engineer. You can diagnose compilation errors, Spring context failures, test assertion failures, and Maven dependency issues in a Spring Cloud Stream micro-integration connector application and apply targeted repairs. You have deep knowledge of Maven build lifecycles, Spring Boot auto-configuration, Testcontainers, the Spring Cloud Stream multi-binder architecture, and the Solace connector framework (`pubsubplus-connector-framework`).

## Overview

Compile and run integration tests for a generated micro-integration module. If any phase fails, classify the error, attempt a targeted repair, and retry. The build-test-repair loop runs up to 5 attempts. On success, print a structured summary. On exhaustion, print a diagnostic and ask the developer for guidance.
...
| `INTEGRATION_APP_ARTIFACT_ID` | Maven module name for the micro-integration application |
| `MI_PACKAGE` | Java package for all micro-integration source files |
...
| `BINDER_ARTIFACT_ID` | Binder module directory name — used to locate the binder design document for repair context |
...
|---|---|---|
| `MI_PACKAGE_PATH` | `MI_PACKAGE` with dots replaced by `/` | |
| `MI_TARGET_MODULE_DIR` | `{TARGET_PROJECT_FOLDER}/{INTEGRATION_APP_ARTIFACT_ID}` | |
| `MI_TARGET_SRC_DIR` | `{MI_TARGET_MODULE_DIR}/src/main/java/{MI_PACKAGE_PATH}` | |
| `MI_TARGET_TEST_SRC_DIR` | `{MI_TARGET_MODULE_DIR}/src/test/java/{MI_PACKAGE_PATH}` | |
| `BINDER_CLAUDE_MD` | `{TARGET_PROJECT_FOLDER}/{BINDER_ARTIFACT_ID}/CLAUDE.md` | Binder design document — additional repair context for binder API details |
...
1. `{MI_TARGET_MODULE_DIR}` directory exists.
2. `{MI_TARGET_MODULE_DIR}/pom.xml` exists.
3. `{MI_TARGET_MODULE_DIR}/CLAUDE.md` exists — this is the design document needed for repair context.
...
Run `mvn test-compile` scoped to the micro-integration module **with `-am`** (also make dependencies). The `-am` flag is required because the micro-integration module depends on the binder and test-support modules within the same project. Use a 5-minute timeout (300 000 ms).

**If `HOST_OS` is `windows`:**
```
cmd.exe //c "set JAVA_HOME={JDK_PATH}&& cd /d {TARGET_PROJECT_FOLDER} && .\mvnw.cmd test-compile -pl {INTEGRATION_APP_ARTIFACT_ID} -am -s maven\settings.xml" 2>&1
```

**If `HOST_OS` is `linux`:**
```
JAVA_HOME="{JDK_PATH}" {TARGET_PROJECT_FOLDER}/mvnw test-compile -pl {INTEGRATION_APP_ARTIFACT_ID} -am -s maven/settings.xml 2>&1
```
...
Run `mvn verify` scoped to the micro-integration module **with `-am`**. Use a 10-minute timeout (600 000 ms) — Testcontainers may pull Docker images on first run.

**If `HOST_OS` is `windows`:**
```
cmd.exe //c "set JAVA_HOME={JDK_PATH}&& cd /d {TARGET_PROJECT_FOLDER} && .\mvnw.cmd verify -pl {INTEGRATION_APP_ARTIFACT_ID} -am -s maven\settings.xml" 2>&1
```

**If `HOST_OS` is `linux`:**
```
JAVA_HOME="{JDK_PATH}" {TARGET_PROJECT_FOLDER}/mvnw verify -pl {INTEGRATION_APP_ARTIFACT_ID} -am -s maven/settings.xml 2>&1
```
...
| **Compilation error** | `COMPILATION ERROR`, `cannot find symbol`, `package does not exist`, `incompatible types`, `method does not override` | Read the compiler error to identify the source file and line number. Read the source file. Read `{MI_TARGET_MODULE_DIR}/CLAUDE.md` Source Transformation Guide section for the correct API and transformation rules. If CLAUDE.md is insufficient, read the technology overview report at `{TECH_OVERVIEW_PATH}` or when it is not sufficient read the binder CLAUDE.md at '{BINDER_CLAUDE_MD}'. Identify the corresponding ABC template file under `abc-micro-integration/` for the structural pattern. Fix the source file.                                                |
| **Spring context failure** | `BeanCreationException`, `NoSuchBeanDefinitionException`, `UnsatisfiedDependencyException`, `ApplicationContextException`, `Failed to load ApplicationContext` | Read the stack trace to identify the failing bean and class. Read `MicroIntegrationApplication.java` under `{MI_TARGET_SRC_DIR}` for bean registrations and `@SpringBootApplication(exclude=...)`. Read the capabilities factory classes under `{MI_TARGET_SRC_DIR}`. Cross-reference with CLAUDE.md Source Transformation Guide — MicroIntegrationApplication section and Capabilities Factories section, and the ABC template's equivalent files under `abc-micro-integration/src/main/java/`. Fix the wiring issue.                                       |
| **Test assertion failure** | `AssertionError`, `AssertionFailedError`, `expected:`, `but was:`, `Expecting`, test method name in stack trace | Read the failing test class source and the test output. Read `{MI_TARGET_MODULE_DIR}/CLAUDE.md` Test Java Plan section. Determine whether the problem is in the test expectations (wrong SDK accessor, wrong destination name, wrong assertion), in connection property injection (`BaseTest.java` — wrong property key or wrong getter call in `setupBinderConnectionProperties()`), or in application configuration (test profile YAML has wrong binder name, wrong binding assignments, or missing extended properties). Fix the appropriate source file. |
...
| **Unknown / unclassifiable** | None of the above patterns match | Print the full error output. Increment the attempt counter. On the next attempt, perform a broader analysis: re-read `{MI_TARGET_MODULE_DIR}/CLAUDE.md`, the overview report at `{TECH_OVERVIEW_PATH}`, and the ABC template equivalent file under `abc-micro-integration/` to look for discrepancies with the generated code.                                                                                                                                                                                                                               |
...
3. **Verify alignment** — after each repair, re-read CLAUDE.md and the ABC template equivalent under `abc-micro-integration/` to confirm the fix is consistent with both.
...
```
## Micro-Integration Verification: PASSED
...
```
## Micro-Integration Verification: FAILED — developer guidance needed
...
```
## Micro-Integration Verification: FAILED — test infrastructure unavailable
...
3. Re-run `/verify-microintegration` after resolving the infrastructure issue
```
